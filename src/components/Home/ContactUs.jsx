...
const LocationIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" className="shrink-0 text-white"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"></path></svg>
);
const PhoneIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" className="shrink-0 text-white"><path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56a.977.977 0 0 0-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.03 21c.53 0 .96-.4.96-.94v-3.71c0-.53-.42-.97-.98-.97z"></path></svg>
);
const EnvelopeIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" className="shrink-0 text-white"><path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm-.4 4.25l-7.07 4.42c-.32.2-.74.2-1.06 0L4.4 8.25a.85.85 0 1 1 .9-1.44L12 11l6.7-4.19a.85.85 0 1 1 .9 1.44z"></path></svg>
);
const ClockIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" className="shrink-0 text-white"><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8z"></path><path d="M12.5 7H11v6l5.25 3.15.75-1.23-4.5-2.67z"></path></svg>
);
const PaperPlaneIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="rotate-45 transition-transform duration-300 group-hover:-translate-y-1 group-hover:translate-x-1"><line x1="22" y1="2" x2="11" y2="13"></line><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon></svg>
);
...
function ContactUs() {
  ...
  const handleSubmit = async(e) => {
    ...
    try {
      ...
      if (res.status === 200 || res.data.success) {
        setToast({ show: true, message: 'Message sent! We will get back to you soon from Ratnamik Metal India.', type: 'success' });
        setFormData({ fullName: '', email: '', phone: '', subject: '', message: '' });
      }
    } catch (error) {
      setToast({ show: true, message: 'Something went wrong. Please try again later.', type: 'error' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const contactItems = [
    { Icon: LocationIcon, label: 'Address', content: 'Office No.8, 1st Floor, Aayesha Manzil, Bldg. No. 81/83, 4th Kumbharwada,Mumbai Maharashtra - 400004, India' },
    { Icon: PhoneIcon, label: 'Phone', content: ['+91 9833864009'] },
    { Icon: EnvelopeIcon, label: 'Email', content: 'info@ratnamikmetal.com' },
    { Icon: ClockIcon, label: 'Working Hours', content: 'Mon - Sat: 9:00 AM - 6:00 PM' }
  ];
  ...
  return (
    <>
      ...
      <CustomToast show={toast.show} message={toast.message} type={toast.type} onClose={() => setToast(prev => ({ ...prev, show: false }))} />
        ...
        <div className="max-w-[1300px] mx-auto">
          ...
          <FadeUpSection isVisible={isVisible} delay={0}>
            <div className="flex flex-col items-center justify-center mb-16 md:mb-20 text-center">
              <h2 className="text-4xl md:text-5xl lg:text-[54px] font-bold text-[#06367b] leading-tight mb-6 tracking-tight">Contact Us</h2>
              <div className={`relative h-1.5 bg-[#06367b] rounded-full overflow-hidden transition-all duration-[1000ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${isVisible ? 'w-24 md:w-32 delay-[200ms]' : 'w-0'}`}>
                  ...
                  <form className="flex flex-col gap-8" onSubmit={handleSubmit}>
                    ...
                    <div className="flex justify-start mt-2">
                      <button type="submit" disabled={isLoading} className={`group relative inline-flex items-center justify-center gap-3 px-10 py-4 font-bold text-white rounded-full overflow-hidden shadow-[0_4px_20px_rgba(46,196,255,0.2)] hover:shadow-[0_8px_30px_rgba(46,196,255,0.4)] transition-all duration-300 ${isLoading ? 'opacity-90 cursor-not-allowed' : 'hover:-translate-y-1'}`}>
                        ...
                        <span className="relative z-10 flex items-center gap-3 tracking-wide">{isLoading ? <><SpinnerIcon /> Sending...</> : <><PaperPlaneIcon /> Send Message</>}</span>
                      </button>
                    </div>
                  </form>
                </div>
                  ...
                  <div className="flex flex-col gap-8 md:gap-10 relative z-10 mb-10">
                    {contactItems.map((item, idx) => (
                      <div key={idx} className="flex flex-row items-start gap-5 group">
                        <div className="mt-1 transition-transform duration-300 group-hover:scale-110 group-hover:text-[#2EC4FF]"><item.Icon /></div>
                        <div className="flex flex-col gap-1.5 flex-1 pt-1">
                          <span className="text-sm font-semibold tracking-wider text-white/80 uppercase">{item.label}</span>
                          {Array.isArray(item.content) ? (
                            <div className="flex flex-col gap-1">
                              {item.content.map((sub, sidx) => (
                                <a key={sidx} href={`tel:${sub.replace(/\s+/g, '')}`} className="text-base text-white hover:text-[#2EC4FF] transition-colors leading-relaxed">{sub}</a>
                              ))}
                            </div>
                          ) : item.label === 'Email' ? (
                            <a href={`mailto:${item.content}`} className="text-base text-white hover:text-[#2EC4FF] transition-colors leading-relaxed">{item.content}</a>
                          ) : (
                            <p className="text-base text-white leading-relaxed">{item.content}</p>
                          )}
                        </div>
                      </div>
                    ))}
