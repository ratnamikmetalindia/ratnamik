...
function Navbar() {
  ...
  const pathname = usePathname(); // Allows us to know if we are on the homepage

  // --- SMART SCROLL HANDLER ---
  const handleNavClick = (e, path) => {
    // If the link is a hash link AND we are currently on the homepage
    if (path.startsWith('/#') && pathname === '/') {
      ...
      if (element) {
        // Force the browser to scroll smoothly
        element.scrollIntoView({ behavior: 'smooth' });
        // Update the URL in the address bar silently
  ...
  // Reusable link style for desktop (No active state, gray default, blue on hover)
    ...
    <>
      ...
      <div className="bg-[#075ca6] w-full relative overflow-hidden text-white py-5 px-4 md:px-[10%] flex flex-col md:flex-row justify-between items-center text-[13px] md:text-base gap-3 md:gap-0 z-[10]">
        ...
        <div className="flex items-center gap-4 md:gap-6 flex-wrap justify-center relative z-10">
          {/* Keep external links as standard <a> tags */}
      ...
      {/* Main Navbar - sticky top-0 keeps it visible */}
      <header className="bg-[#ffffff] h-[90px] w-full flex justify-between px-4 md:px-0 sticky top-0 z-[999] shadow-sm">
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-[90px] left-0 w-full bg-[#D1D1D1] flex flex-col z-[100] shadow-2xl border-t border-gray-300 max-h-[calc(100vh-90px)] overflow-y-auto overflow-x-hidden pb-32 overscroll-contain [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
